class UIUtils {
  ...
  static showToastSuccess (message, duration = 1000) {
    const instance = showNotify({
      type: 'success', message: message, duration: duration, onClick: (event) => {
        instance?.close()
      },
    })
  }
  ...
  static showToastError (message, duration = 3000) {
    const instance = showNotify({
      type: 'danger', message: message, duration: duration, onClick: (event) => {
        instance?.close()
      },
    })
  }
  ...
  static showLoadingWhen (isLoading) {
    ...
    watch(() => isLoading.value, async (newValue, oldValue) => {
      if (newValue) {
