  ...
  <div class="app-form">
    ...
    <van-form ref="form" class="" @submit="saveItem" @failed="onValidationError">
      <van-cell-group inset>
        ...
        <tag-select v-model="parentTag" label="Parent tag" :is-multi-select="false" />

        <icon-select v-model="icon" />

        <app-boolean v-model="isTodo" label="Mark as to-do">
          <template #label>
...
const fetchItem = async () => {
  const tag = cloneDeep(dataStore.tagDictionaryById[itemId.value])
...
const onEvent = (event, payload) => {
  if (event === 'onPostSave') {
    ...
    const oldTagsList = dataStore.tagList
      .map((item) => (isTodo && set(item, 'attributes.is_todo', false)) || item)
      .filter((item) => item.id !== itemId.value)
