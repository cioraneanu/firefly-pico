export const LANGUAGE_ENGLISH = 'en-US'
export const LANGUAGE_ROMANIAN = 'ro-RO'
export const LANGUAGE_FRENCH = 'fr-FR'
export const LANGUAGE_RUSSIAN = 'ru-RU'

export const OPTION_ENGLISH = { code: LANGUAGE_ENGLISH, name: 'English', flag: 'en' }
export const OPTION_ROMANIAN = { code: LANGUAGE_ROMANIAN, name: 'Romanian', flag: 'ro' }
export const OPTION_FRENCH = { code: LANGUAGE_FRENCH, name: 'French', flag: 'fr' }
export const OPTION_RUSSIAN = { code: LANGUAGE_RUSSIAN, name: 'Russian', flag: 'ru' }

export const OPTIONS_LIST = [OPTION_ENGLISH, OPTION_ROMANIAN, OPTION_FRENCH, OPTION_RUSSIAN]
