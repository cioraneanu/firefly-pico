      ...
      <div ref="popupContentRef" class="flex-1 flex-column overflow-auto color" style="padding-bottom: 100px">
        ...
        <van-form @submit="onApplyFilters">
          ...
          <app-button-form-save label="Apply filters" bottom="20">
            <template #left>
              <van-button v-if="isFiltered" @click="onClearFilters" round>
                Clear
              </van-button>
            </template>
...
const {
  description, dateStart, dateEnd, amountStart, amountEnd,
  category, withoutCategory, tag, withoutTag, account, transactionType,
} = generateChildren(localModelValue, [
  'description', 'dateStart', 'dateEnd', 'amountStart', 'amountEnd',
  'category', 'withoutCategory', 'withoutTag', 'tag', 'account', 'transactionType',
])
