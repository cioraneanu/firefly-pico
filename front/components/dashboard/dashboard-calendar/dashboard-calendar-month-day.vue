...
const amountIncome = computed(() => {
  const value = get(dashboardStats.dashboardCalendarTransactionsByDate, `${formattedDate.value}.${Transaction.types.income.code}`)
  return value ? formatNumberForDashboard(value) : null
})

const amountExpense = computed(() => {
  const value = get(dashboardStats.dashboardCalendarTransactionsByDate, `${formattedDate.value}.${Transaction.types.expense.code}`)
  return value ? formatNumberForDashboard(value) : null
})

const amountTransfer = computed(() => {
  const value = get(dashboardStats.dashboardCalendarTransactionsByDate, `${formattedDate.value}.${Transaction.types.transfer.code}`)
  return value ? formatNumberForDashboard(value) : null
})
