...
const expenseAccountList = computed(() =>
  accountList.value.filter((item) => isEqual(Account.getType(item), Account.types.expense)),
)
const assetAccountList = computed(() =>
  accountList.value.filter((item) => isEqual(Account.getType(item), Account.types.asset)),
)
const incomeAccountList = computed(() =>
  accountList.value.filter((item) => isEqual(Account.getType(item), Account.types.revenue)),
)

const showAssets = computed(
  () => props.allowedTypes.some((item) => isEqual(item, Account.types.asset)) && assetAccountList.value.length > 0,
)
const showExpense = computed(
  () => props.allowedTypes.some((item) => isEqual(item, Account.types.expense)) && expenseAccountList.value.length > 0,
)
const showIncome = computed(
  () => props.allowedTypes.some((item) => isEqual(item, Account.types.revenue)) && incomeAccountList.value.length > 0,
)
