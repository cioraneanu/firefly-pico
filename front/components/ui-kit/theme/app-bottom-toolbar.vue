<template>
  <van-tabbar @change="onChange" v-model="activeTab" :safe-area-inset-bottom="true" :fixed="true">
    ...
    <van-tabbar-item
      :name="tabConstants.settings"
      @click="onChange(tabConstants.settings)"
      :dot="appStore.isNewVersionAvailable"
    >
      Settings
      <template #icon="{ active }">
...
watch(
  ...
  (newValue) => {
    if (
      newValue === RouteConstants.ROUTE_TRANSACTION_LIST ||
      RouteConstants.isForm(RouteConstants.ROUTE_TRANSACTION_ID, newValue)
    ) {
      activeTab.value = tabConstants.transactionList
    }

    if (
      newValue === RouteConstants.ROUTE_TRANSACTION_TEMPLATE_LIST ||
    ...
    if (
      [
        ...
        RouteConstants.ROUTE_CATEGORY_LIST,
      ].includes(newValue) ||
