...
2. Select your docker-compose:

    1. If you already have "Firefly III" installed you can choose from one of the DB flavors below:  
       [Pico with Sqlite ](../docker-compose.pico-simple.yml) or [Pico with Postgres ](../docker-compose.pico.yml)
        - Change `FIREFLY_URL`

    2. If you want 2 in 1: "Firefly III" + "Firefly-Pico": [Pico + Firefly](../docker-compose.pico+firefly.yml)
        - Change `FIREFLY_URL` and `APP_URL`

    3. If you want 3 in 1: "Firefly III" + "Firefly Importer" + "Firefly-Pico"  
       [Pico + Firefly + Importer](../docker-compose.pico+firefly+importer.yml)
        - Change `FIREFLY_URL`, `APP_URL` and `FIREFLY_III_URL`

NOTE: You can use also use SQLite for option 2/3 just like in th 1st example to keep things short and simple

> [!NOTE]  
> `FIREFLY_URL` needs to point your **Firefly III** instance. If you don't use a reverse proxy make sure you provide the full IP + Port.  
